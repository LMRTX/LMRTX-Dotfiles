...
**I built this `MangoWM` setup with (in my opinion) sane defaults and a `hyprland` like dynamic feel using dwindle-tiling as the default while still being lightweight for a `Wayland` compositor. The setup was made to be close to `hyprland` so this is not for someone looking for a `dwm`-like feel**
